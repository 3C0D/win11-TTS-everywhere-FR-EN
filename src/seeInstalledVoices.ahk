...
try {
    ...
    regInfo .= "Erreur lors de la lecture du registre: " . e.Message . "`n"
}
...
Tabs.UseTab(3)
MyGui.Add("Text", "w580", "Sélectionnez une voix et testez-la:")
...
TestText := MyGui.Add("Edit", "r5 w580 vTestText", "Ceci est un test de synthèse vocale. This is a text-to-speech test.")
...
CopyAllInfo(*)
{
    ...
    if (fullInfo = "") {
        fullInfo := "Aucune information à copier dans cet onglet."
    }
    
    A_Clipboard := fullInfo
    MsgBox("Informations copiées dans le presse-papiers !")
}
...
ScanMissingVoices(*)
{
    ...
    diagnosticInfo := "Recherche de fichiers de voix dans Windows...`n`n"
            ...
            try {
                Loop Files, path . "*.dll"
                {
                    if InStr(A_LoopFileName, "TTS") || InStr(A_LoopFileName, "Speech") {
                        ...
                        diagnosticInfo .= "  Trouvé: " . A_LoopFileName . "`n"
                    }
                }
            } catch as e {
                diagnosticInfo .= "  Erreur: " . e.Message . "`n"
            }
        } else {
    ...
    diagnosticInfo .= "`nRecherche approfondie dans les dossiers système...`n"
    ...
    diagnosticInfo .= "`nComparaison avec les voix enregistrées...`n"
    registeredVoices := []
    ...
    try {
        ...
        diagnosticInfo .= "Erreur lors de la lecture du registre: " . e.Message . "`n"
    }
    ...
    for regPath in alternativeRegPaths {
        ...
        try {
            ...
            {
                count++
                diagnosticInfo .= "  Trouvé: " . A_LoopRegName . "`n"
            }
    ...
    if (foundVoiceFiles.Length = 0) {
        diagnosticInfo .= "- Les fichiers de voix semblent manquants. Essayez de réinstaller les packs de voix depuis les paramètres Windows.`n"
        diagnosticInfo .= "- Allez dans Paramètres > Temps et langue > Langue et région > [votre langue] > Options > Synthèse vocale`n"
    } else if (registeredVoices.Length < 4) {
        diagnosticInfo .= "- Certaines voix sont présentes mais pas correctement enregistrées. Utilisez le bouton 'Tenter de réenregistrer les voix'.`n"
        diagnosticInfo .= "- Vous pouvez aussi essayer d'installer des voix supplémentaires depuis les paramètres Windows.`n"
    }
...
AttemptToRegisterVoices(*)
{
    ...
    diagnosticInfo .= "`nOpération terminée. Veuillez redémarrer l'application pour vérifier si de nouvelles voix sont disponibles.`n"
    diagnosticInfo .= "Si le problème persiste, vous pourriez avoir besoin de réinstaller les packs de voix depuis les paramètres Windows.`n"
